...
double phase(){
    ...
    double delta = daysSinceNewMoon(currentTime.tm_year+1900,currentTime.tm_yday,currentTime.tm_hour);
...
void hour_layer_update_callback(Layer *me, GContext* ctx) {
    ...
    int p = floor(16.*(phasePercent()+0.09));
    p = (p % 16)/2;
    ...
    int c3 = (c1==c2)?cols[p][2]:c2;
    
    double angle = TRIG_MAX_ANGLE/-4.+TRIG_MAX_ANGLE * (min/60.+hour)/12.;
    ...
    graphics_draw_line(
        ...
        GPoint(
            centerx + (c*radius*houLen)+ox, 
            centery + (s*radius*houLen)+oy));
    ...
    graphics_draw_line(
        ...
        GPoint(
            centerx + (c*radius*houLen), 
            centery + (s*radius*houLen)));
    
    graphics_context_set_fill_color(ctx, cols[p][0]);
    graphics_fill_circle(ctx, GPoint(centerx,centery), 3);
}
...
void minute_layer_update_callback(Layer *me, GContext* ctx) {
    ...
    double angle = TRIG_MAX_ANGLE/-4.+TRIG_MAX_ANGLE * min/60.;
    double c = cos_lookup(angle) / 65536.;
    double s = sin_lookup(angle) / 65536.;
    ...
    graphics_draw_line(
        ...
        GPoint(centerx+ox, centery+oy), 
        GPoint(
            centerx + (c*radius*minLen)+ox, 
            centery + (s*radius*minLen)+oy));
    ...
    graphics_draw_line(
        ...
        GPoint(centerx, centery), 
        GPoint(
            centerx + (c*radius*minLen), 
            centery + (s*radius*minLen)));
...
void second_layer_update_callback(Layer *me, GContext* ctx) {
    ...
    graphics_draw_line(
        ...
        GPoint(centerx+ox, centery+oy), 
        GPoint(
            centerx + (c*(radius*secLen+2*secRad))+ox, 
            centery + (s*(radius*secLen+2*secRad))+oy));
    ...
    graphics_draw_line(
        ...
        GPoint(centerx, centery), 
        GPoint(
            centerx + (c*(radius*secLen+2*secRad)), 
            centery + (s*(radius*secLen+2*secRad))));
    
    double cc = (radius + 2*secRad)*secLen;
